---
publish: true
permalink: /2-cards/How Power Users Actually Set Up Claude Code 中英對照筆記.md
created: 2026-04-18
modified: 2026-04-18
tags:
  - 主題/AI與知識工作流
...
>
> 原 clipping：[[How Power Users Actually Set Up Claude Code]]\(4-clippings/)
...
> _"Pull all deals that haven't moved in 21 days and summarise what's stalling by stage and suggest plays to unblock them."_ → HubSpot MCP.
>
> _"Which of our active paid campaigns has a CAC above target this month?"_ → Google Ads MCP.
>
> _"Go to \[competitor]'s pricing page, capture what's there, compare it against the snapshot saved in /research/competitor-snapshots/, and flag anything that's changed."_ → Playwright MCP.
...
> _「把過去 21 天沒動的所有 deal 撈出來,按 stage 總結卡在哪、建議怎麼解。」_ → HubSpot MCP
>
> _「我們 active 的付費 campaign,這個月哪些 CAC 超過目標?」_ → Google Ads MCP
>
> _「去 \[對手] 的 pricing 頁面抓快照,跟 `/research/competitor-snapshots/` 裡的舊快照比對,flag 任何改變。」_ → Playwright MCP
...
- [[Claude Code 指令系統 Source of Truth]] — 我的底層架構 SOT,比對分析見下條
- [[Kieran Flanagan 四大模組 × Source of Truth 比對分析]] — 本卡的後續分析(本次建立)
- [[Claude Code 四大模組 — Context × Rules × Reach × Operation]] — 2026-04-17 既有的四模組概念卡(MKT1 Get 筆記版),跟本卡互補:那張是摘要 + 現況對照,本卡是逐段中英對照原文
- [[Kieran 五原則深度解讀 — 從個人 Skill 走向團隊系統]] — Kieran 其他文章的深度解讀
